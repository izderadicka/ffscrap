...
function applyScraper(scraper) {
	...
	if (errors) {
		...
		worker=theTab.attach({
			contentScriptFile : [data.url('jquery-2.1.4.min.js'), data.url('center.js')],
	...
	if (tabs.activeTab.scraperDef) {
		if (tabs.activeTab.scraperDef.length === 1 && ! onNew) {
			...
			var selectPanel=createPanel({
				...
				contentScriptFile:[data.url('jquery-2.1.4.min.js'), data.url('chooserPanel.js')]
			});
...
var menuPanel = createPanel({
	...
	contentScriptFile:[data.url('jquery-2.1.4.min.js'), data.url('menuPanel.js')],
...
var resultPanel = createPanel({
	...
	contentScriptFile : [data.url('jquery-2.1.4.min.js'), data.url('resultPanel.js')]
});
...
var errorPanel = createPanel({
	...
	contentScriptFile : [data.url('jquery-2.1.4.min.js'), data.url('errorPanel.js')]
});
...
var editPanel = createPanel({
	...
	contentScriptFile : [data.url('jquery-2.1.4.min.js'), data.url('scraperPanel.js')]
});
